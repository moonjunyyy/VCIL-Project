...
class Ours_test(_Trainer):
    ...
    def cp_loss(self,feat,y,batch_g,total_batch_g):
        ...
        peeking_w = self.model.backbone.fc.weight[:len(self.exposed_classes)] - self.lr*self.beta*total_batch_g.detach()    #* B,dim
...
class baseline(_Trainer):
    ...
    def model_forward(self, x, y,ref_fc):
        with torch.cuda.amp.autocast(enabled=self.use_amp):
            ...
            logit = self.model(x)
            logit = logit + self.mask
            loss = self.sample_criterion(logit, y).mean()
